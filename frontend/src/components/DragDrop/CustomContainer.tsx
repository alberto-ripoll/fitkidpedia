...
export function CustomDragDrop({
  ...
  function handleDrop(e: any, type: string) {
    ...
    const allFilesValid = files.every((file: File) => {
      return formats.some((format) =>
        file.type.toUpperCase().endsWith(`/${format.toUpperCase()}`)
      );
    });
    ...
    if (files && files.length) {
      const nFiles = files.map(async (file: File) => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (dropContainer.current) {
        ...
        dropContainer.current.removeEventListener(
          "dragleave",
          handleDragLeave
        );
      }
    ...
    <>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="text-[12px] font-normal text-gray-500">
            <input
              ...
              accept="video/*"
              ref={fileRef}
              onChange={(e) => handleDrop(e, "inputVideo")}
            />
            <span
      ...
      {ownerLicense.length > 0 && (
        <div className="mt-4 grid grid-cols-2 gap-y-4 gap-x-4">
          {ownerLicense.map((img: any, index: number) => (
